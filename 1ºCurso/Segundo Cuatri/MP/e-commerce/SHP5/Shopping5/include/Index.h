...
private:
    ...
    unsigned _capacity; // Number of current allocated position in memory
    ...
    /**
     * @brief reserve memory for n events
     * @return beginning of the reserved direction
     */
    Pair * allocate(int capacidad);
    
    /**
     * @brief Add memory for the Pair vector
     */
    void reallocate(int new_tama);
    ...
    void deallocate();
    /**
     * @brief copy the Event vector to the member _events
     * @param one Event vector from which _events is copied
     */
    void copy(Pair *dest);
...
public:
    ...
    /**
     * @brief Informs on which field is mounted the index, Brand 1 UserID 0
     * @return the string Brand,UserID
     */
    std::string getSOnWhich(int onBrand = npos) const;
     ...
     */
    void initDefault(int onBrand = 0);
     ...
     * 
     */
    int add(const Pair & pair, bool orden = 1);
    ...
    /**
     * @brief copy constructor, a new object is created from the parametred object
     * @param orig the index we want a copy from
     */
    Index(const Index & orig);
    
    /**
     * Assigment operator overload. This operator copies the content of one Index into this.
...
int BusquedaBinaria (const Pair pair[], std::string a_buscar, const int util);
